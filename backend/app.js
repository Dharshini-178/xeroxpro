...
// ─── Print Job Routes (with file upload) ───────────────────────────────────────

// Submit a print job WITH file upload
app.post('/api/print-jobs', upload.single('file'), async (req, res) => {
  try {
    ...
    const jobData = {
      ...
      pages: Number(pages),
      file_name: fileName || (req.file ? req.file.originalname : null),
    };

    // If a file was uploaded, store it in MongoDB
    if (req.file) {
      jobData.file_data = req.file.buffer;
      jobData.file_mimetype = req.file.mimetype;
      jobData.file_size = req.file.size;
    }
